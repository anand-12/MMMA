...
init_x = torch.tensor(init_x, requires_grad=True).float()
init_y = torch.tensor(init_y, requires_grad=True).float().squeeze()
...
def get_next_points(init_x, init_y, best_init_y, bounds, n_points=1, gains=None):
    ...
    single_model.fit(init_x.requires_grad_(), init_y)
